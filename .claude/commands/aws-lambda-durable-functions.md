...
Load the appropriate reference file (using the Read tool) based on what the user is working on. Files are located at `.claude/skills/aws-lambda-durable-functions/steering/`:

- **Getting started**, **basic setup**, **example**, **ESLint**, or **Jest setup** -> read `getting-started.md`
- **Understanding replay model**, **determinism**, or **non-deterministic errors** -> read `replay-model-rules.md`
- **Creating steps**, **atomic operations**, or **retry logic** -> read `step-operations.md`
- **Waiting**, **delays**, **callbacks**, **external systems**, or **polling** -> read `wait-operations.md`
- **Parallel execution**, **map operations**, **batch processing**, or **concurrency** -> read `concurrent-operations.md`
- **Error handling**, **retry strategies**, **saga pattern**, or **compensating transactions** -> read `error-handling.md`
- **Advanced error handling**, **timeout handling**, **circuit breakers**, or **conditional retries** -> read `advanced-error-handling.md`
- **Testing**, **local testing**, **cloud testing**, **test runner**, or **flaky tests** -> read `testing-patterns.md`
- **Deployment**, **CloudFormation**, **CDK**, **SAM**, **log groups**, **deploy**, or **infrastructure** -> read `deployment-iac.md`
- **Advanced patterns**, **GenAI agents**, **completion policies**, **step semantics**, or **custom serialization** -> read `advanced-patterns.md`
- **Troubleshooting**, **stuck execution**, **failed execution**, **debug execution ID**, **execution history**, **execution error**, **why did my execution fail**, **execution timed out**, **callback not received**, **diagnose execution**, or **root cause execution** -> read `troubleshooting-executions.md`
